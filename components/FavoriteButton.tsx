...
const FavoriteButton: React.FC<FavoriteButtonProps> = ({ movieId }) => {
  ...
  const addFavorites = useCallback(async () => {
    const response = await axios.post('/api/favorite', { movieId, isChild: isChild, childName: router.query.childUsername });
  ...
  const deleteFavorites = useCallback(async () => {
    const response = await axios.delete('/api/favorite', { data: { movieId, isChild: isChild, childName: router.query.childUsername } });
    const updatedFavoritesId = response?.data?.favoritesId;
  ...
  return (
    <div className='flex flex-row gap-4'>
      <div onClick={addFavorites} className="cursor-pointer group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300">
        <AiOutlinePlus className="text-white group-hover/item:text-neutral-300 w-4 lg:w-6" />
       
      </div>
      <div onClick={deleteFavorites} className="cursor-pointer group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300">
