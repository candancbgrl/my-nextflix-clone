...
const MainThumb = () => {
  ...
  const playVideo = useCallback(
    ...
    [router, randomMovie?.id]
  );

  return (
    <div className="relative">
      <video
        className="w-full h-[69vw] object-cover brightness-[60%] transition duration-500"
        autoPlay
        muted
        loop
      ...
      <div className="absolute top-[20rem] left-0 left-4 flex flex-col ">
        ...
        <p className="text-white text-md max-w-[45rem] mt-6">
          {randomMovie?.description}
        </p>
        <div className="flex flex-row gap-8 mt-8">
          <div
            onClick={playVideo}
            className="bg-white rounded-md flex flex-row gap-3 justify-center items-center px-5 py-3
                    pointer-cursor hover:bg-gray-300"
          >
            <FaPlay className="text-black " fontSize={20} />
            <button className="text-black font-bold ">Play</button>
          </div>
          <div
            className="bg-zinc-500 bg-opacity-50 rounded-md flex flex-row justify-center items-center px-4
                    hover:bg-opacity-30"
          >
            <AiOutlineInfoCircle className="text-white " fontSize={25} />
            <button
              onClick={handleOpenModal}
              className="text-white opacity-100 p-2 font-bold"
            >
              More Info
            </button>
          </div>
